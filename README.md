...
#### 3.1.2 配置文件加载逻辑
程序会按以下优先级查找配置文件：
1. 开发环境路径：当前目录、上级目录的 services.dat
...
## 9. 故障排除
### 9.1 常见问题
1. **配置文件加载失败**：检查 services.dat 路径和格式
2. **进程无法终止**：以管理员身份运行程序
...
2. 检查控制台输出的配置加载路径
3. 验证 services.dat 的 JSON 格式合法性
4. 确认服务可执行文件路径和权限
